...
class TopGainersLosersStrategy(CtaTemplate):
    ...
    def on_tick(self, tick: TickData):
        ...
        if self.indicator_inited and not self.target_pos and not self.stop_open and not self.closed:
            open_allowed = False
            if self.direction == Direction.SHORT and tick.last_price <= self.hour_up - ((self.hour_up - self.hour_down) / 20) and tick.last_price >= self.hour_up - ((self.hour_up - self.hour_down) / 3):
                open_allowed = True

            if self.direction == Direction.LONG and tick.last_price >= self.hour_down + ((self.hour_up - self.hour_down) / 20) and tick.last_price <= self.hour_down + ((self.hour_up - self.hour_down) / 3):
                open_allowed = True
    ...
    def add_unit_pos(self, tick_price: float):
        ...
        # 更新止盈价格
        if self.direction == Direction.LONG:
            self.profit_price = tick_price * (1 + (abs((self.hour_down / tick_price) - 1) * max(abs(self.pnl) / leverage, 3.3)))
            self.profit_price = max(self.profit_price, self.hour_down + ((self.hour_up - self.hour_down) * 0.6))
        
        if self.direction == Direction.SHORT:
            self.profit_price = tick_price * (1 - (abs((self.hour_up / tick_price) - 1) * max(abs(self.pnl) / leverage, 3.3)))

                ...
                if self.direction == Direction.LONG:
                    ...
                    if self.hour_up and self.hour_down and self.minute_15_up and self.minute_15_down and self.minute_recent_up and self.minute_recent_down:
                        if signal_name == "T1":
                            ...
                            if signal.pre_signal_dt and 3 <= recent_minutes <= 30 and self.hour_up - signal.pre_signal_hour_up <= abs(self.hour_up - signal.pre_signal_hour_down) / 4.0 and stop_price and stop_price >= self.hour_up - abs(self.hour_up - signal.pre_signal_hour_down) / 4.0 and self.minute_recent_down >= self.hour_down:
                                indicator_inited = True

                        elif signal_name == "T5":
                            if 3 <= recent_minutes <= 30 and self.minute_recent_down >= self.hour_down:
                                indicator_inited = True
                ...
                if self.direction == Direction.SHORT:
                    ...
                    if self.hour_up and self.hour_down and self.minute_15_up and self.minute_15_down and self.minute_recent_up and self.minute_recent_down:
                        if signal_name == "T1":
                            ...
                            if signal.pre_signal_dt and 3 <= recent_minutes <= 30 and signal.pre_signal_hour_down - self.hour_down <= abs(signal.pre_signal_hour_up - self.hour_down) / 4.0 and stop_price and stop_price <= self.hour_down + abs(signal.pre_signal_hour_up - self.hour_down) / 4.0 and self.minute_recent_up <= self.hour_up:
                                indicator_inited = True

                        elif signal_name == "T5":
                            if 3 <= recent_minutes <= 30 and self.minute_recent_up <= self.hour_up:
                                indicator_inited = True

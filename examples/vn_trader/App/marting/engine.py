...
from .martingPortfolio import MartingPortfolio
import re
...
class MartingEngine(BaseEngine):
    ...
    def add_strategy(self, setting):
        ...
        # 创建策略实例
        strategy = strategy_class(self, self.martingPortfolio, setting)
    ...
    def get_portfolio_variables(self):
        ...
        for key in self.martingPortfolio.varList:
            varDict[key] = self.martingPortfolio.__getattribute__(key)
    ...
    def get_portfolio_parameters(self):
        ...
        for key in self.martingPortfolio.paramList:
            paramDict[key] = self.martingPortfolio.__getattribute__(key)
            ...
            if not self.downloading:
                ...
                if self.absolute_generate_needed:
                    ...
                    if result:
                        self.absolute_generate_needed = False
                        # 海龟策略重新初始化
                        self.marting_engine.reinit_strategies()

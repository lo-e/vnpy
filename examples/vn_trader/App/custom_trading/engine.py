...
from .base import APP_NAME, EVENT_CUSTOMTRADING_PORTFOLIO
...
class CustomTradingEngine(BaseEngine):
    engine_type = EngineType.LIVE  # live trading engine

    def __init__(self, main_engine: MainEngine, event_engine: EventEngine):
        """"""
        super(CustomTradingEngine, self).__init__(main_engine, event_engine, APP_NAME)
        ...
        # 组合管理类
        self.customTradingPortfolio: CustomTradingPortfolio = None
    ...
    def init_engine(self):
        ...
        self.customTradingPortfolio = CustomTradingPortfolio(self, portfolio_setting)
    ...
    def process_timer_event(self, event: Event):
        # 投资组合推送
        if self.customTradingPortfolio.inited:
            self.customTradingPortfolio.on_timer()
    ...
    def _init_strategy(self):
        ...
        while not self.init_queue.empty():
            ...
            self.write_log(f"自主交易策略{strategy_name}开始执行初始化")
            ...
            self.put_strategy_event(strategy)
            self.write_log(f"自主交易策略{strategy_name}初始化完成")
    ...
    def start_strategy(self, strategy_name: str):
        ...
        self.put_strategy_event(strategy)
        self.write_log(f"自主交易策略{strategy_name}启动")
    ...
    def put_portfolio_event(self):
        ...
        # 刷新Portfolio组件UI
        event = Event(type=EVENT_CUSTOMTRADING_PORTFOLIO, data=self.get_portfolio_variables())
        self.event_engine.put(event)

    def loadPortfolioSyncData(self):
        """从数据库导入投资组合历史同步数据"""
        syncData = self.main_engine.dbQuery(
            PORTFOLIO_DB_NAME, self.customTradingPortfolio.name, {}
        )
        ...
        d = syncData[0]
        for key in self.customTradingPortfolio.syncs:
            if key in d:
                self.customTradingPortfolio.__setattr__(key, d[key])
    
    def savePortfolioSyncData(self):
        """保存投资组合同步数据到数据库"""
        if not self.customTradingPortfolio:
            return

        d = {}
        for key in self.customTradingPortfolio.syncs:
            d[key] = self.customTradingPortfolio.__getattribute__(key)
        
        if d:
            self.main_engine.dbUpdate(
                PORTFOLIO_DB_NAME,
                self.customTradingPortfolio.name,
                d,
                {},
                True,
    ...
    def portfolioDbUpdateCallback(self, back_data=None):
        try:
            if isinstance(back_data, dict):
                ...
                if result:
                    content = f"跟单交易组合{self.customTradingPortfolio.name}同步数据保存成功"
                else:
                    content = f"跟单交易组合{self.customTradingPortfolio.name}同步数据保存失败"
    ...
    def initPortfolio(self):
        ...
        # 投资组合初始化
        if not self.customTradingPortfolio.inited:
            self.customTradingPortfolio.on_init()
            self.customTradingPortfolio.inited = True
    ...
    def startPortfolio(self):
        ...
        # 投资组合启动
        if not self.customTradingPortfolio.starting:
            self.customTradingPortfolio.on_start()
            self.customTradingPortfolio.starting = True
            self.put_portfolio_event()
    ...
    def stopPortfolio(self):
        ...
        if self.customTradingPortfolio.starting:
            self.customTradingPortfolio.on_stop()
            self.customTradingPortfolio.starting = False
            self.put_portfolio_event()

    def get_portfolio_variables(self):
        varDict = OrderedDict()
        for key in self.customTradingPortfolio.variables:
            varDict[key] = self.customTradingPortfolio.__getattribute__(key)
    ...
    def get_portfolio_parameters(self):
        ...
        for key in self.customTradingPortfolio.parameters:
            paramDict[key] = self.customTradingPortfolio.__getattribute__(key)

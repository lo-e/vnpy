...
class CSVsOKExBarLocalEngine(object):
    def __init__(self, duration:str):
        super(CSVsOKExBarLocalEngine, self).__init__()
        # 周期
        self.duration = duration
        # 项目路径
        self.walkingDir = get_csv_path()
            ...
            for theFile in files:
                ...
                if '.csv' in theFile and dirName == self.duration:
                    if dirName == '1D' or dirName == '1Dutc':
                        db_name = DAILY_DB_NAME
                    elif dirName == '1m' or dirName == '3m' or dirName == '5m' or dirName == '15m' or dirName == '30m':
                        duration = re.sub("\D", "", dirName)
                        db_name = MinuteDataBaseName(duration)
                    else:
                        print('输入的周期不正确！！')
...
class CSVsFTXBarLocalEngine(object):
    def __init__(self, duration:str):
        super(CSVsFTXBarLocalEngine, self).__init__()
        # 周期
        self.duration = duration
        # 项目路径
        self.walkingDir = get_csv_path()

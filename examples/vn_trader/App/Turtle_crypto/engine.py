...
class DingTalkEngine(BaseEngine):
    ...
    def __init__(self, main_engine: MainEngine, event_engine: EventEngine):
        ...
        self.active = False
        self.ai_ding = None
        self.main_engine.ding_talk = self.ding_talk

    def ding_talk(self, content: str):
        """"""
        ...
        while self.active:
            try:
                if not self.ai_ding:
                    ...
                    self.ai_ding = DingtalkChatbot(webhook)

                content = self.queue.get(block=True, timeout=1)
                self.ai_ding.send_text(msg=content, is_at_all=True)
            except:
                pass

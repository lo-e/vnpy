...
from .supportResistancePortfolio import SupportResistancePortfolio
...
class SupportResistanceEngine(BaseEngine):
    ...
    def __init__(self, main_engine: MainEngine, event_engine: EventEngine):
        """"""
        super(SupportResistanceEngine, self).__init__(main_engine, event_engine, APP_NAME)
        ...
        # 组合管理类
        self.portfolio: SupportResistancePortfolio = None
    ...
    def init_engine(self):
        ...
        self.portfolio = SupportResistancePortfolio(self, portfolio_setting)
    ...
    def start_strategy(self, strategy_name: str):
        ...
        self.put_strategy_event(strategy)
        self.write_log(f"支撑压力策略{strategy_name}启动")
    ...
    def put_portfolio_event(self):
        ...
        # 刷新Portfolio组件UI
        event = Event(type=EVENT_SUPPORT_RESISTANCE_PORTFOLIO, data=self.get_portfolio_variables())
    ...
    def savePortfolioSyncData(self):
        ...
        d = {}
        for key in self.portfolio.syncs:
            d[key] = self.portfolio.__getattribute__(key)
    ...
    def portfolioDbUpdateCallback(self, back_data=None):
        try:
            if isinstance(back_data, dict):
                ...
                if result:
                    content = f"支撑压力策略组合{self.portfolio.name}同步数据保存成功"
                else:
                    content = f"支撑压力策略组合{self.portfolio.name}同步数据保存失败"
    ...
    def add_strategy(self, setting):
        ...
        # 创建策略实例
        strategy = SupportResistanceStrategy(self, setting)
    ...
    def strategyDbUpdateCallback(self, back_data=None):
        try:
            if isinstance(back_data, dict):
                ...
                if result:
                    content = f"支撑压力策略策略{strategy_name}同步数据保存成功"
                else:
                    content = f"支撑压力策略策略{strategy_name}同步数据保存失败！！"
                    self.write_log(content)
            else:
                content = f"支撑压力策略策略同步数据保存失败！！"
                self.write_log(content)
        except:
            content = f"支撑压力策略策略同步数据保存失败！！"
            self.write_log(content)
    ...
    def get_portfolio_variables(self):
        ...
        for key in self.portfolio.variables:
            varDict[key] = self.portfolio.__getattribute__(key)
    ...
    def get_portfolio_parameters(self):
        ...
        for key in self.portfolio.parameters:
            paramDict[key] = self.portfolio.__getattribute__(key)

...
class BBreakerStrategy(CtaTemplate):
    """趋势突破策略"""
    className = 'BBreakerStrategy'
    author = u'loe'
    ...
    def __init__(self, ctaEngine, setting):
        """Constructor"""
        super(BBreakerStrategy, self).__init__(ctaEngine, setting)
...
def GetEngin(settingDict, symbol,
    ...
    # 引擎中创建策略对象
    engine.initStrategy(BBreakerStrategy, settingDict)
...
if __name__ == '__main__':
    ...
    start = datetime.now()
    engine.runParallelOptimization(BBreakerStrategy, setting)
    print datetime.now() - start
    #'''

    '''
    # 回测
    engine.strategy.name = 'bbreaker_backtesting'

...
class SimpleTestStrategy(CtaTemplate):
    ...
    def __init__(self, ctaEngine, setting):
        """Constructor"""
        super(SimpleTestStrategy, self).__init__(ctaEngine, setting)
    ...
    def onTick(self, tick):
        ...
        if tick.datetime.time() >= self.stopTime:
            # 超过建仓截止时间，取消所有委托
            self.cancelAll()
            return
        ...
        if self.pos == 0:
            ...
            # 持有多头仓位
            if (tick.lastPrice <= self.highPrice * (1 - self.outPercent / 100)) and ( not self.ordering) and not self.todayEntry and self.trading:
                # 止盈止损
            ...
            # 持有空头仓位
            if (tick.lastPrice >= self.lowPrice * (1 + self.outPercent / 100)) and ( not self.ordering) and not self.todayEntry and self.trading:
                # 止盈止损
...
def GetEngin(settingDict, symbol,
    ...
    # 引擎中创建策略对象
    engine.initStrategy(SimpleTestStrategy, settingDict)
...
if __name__ == '__main__':
    ...
    start = datetime.now()
    engine.runParallelOptimization(SimpleTestStrategy, setting)
    ...
    #'''
    # 回测
    engine.strategy.name = 'simpleTest_backtesting'

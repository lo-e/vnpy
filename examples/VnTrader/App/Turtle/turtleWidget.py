...
from vnpy.trader.app.ctaStrategy.ctaBase import EVENT_CTA_LOG, EVENT_CTA_STRATEGY, EVENT_TURTLE_PORTFOLIO
...
class TurtlePortfolioManager(QtWidgets.QGroupBox):
    ...
    # ----------------------------------------------------------------------
    def __init__(self, ctaEngine, eventEngine, turtleEngine, parent=None):
        """Constructor"""
    ...
    def updateMonitor(self):
        """显示组合最新状态"""
        paramDict = self.ctaEngine.getPortfolioParam()
        if paramDict:
            self.paramMonitor.updateData(paramDict)

        varDict = self.ctaEngine.getPortfolioVar()
        if varDict:
            self.varMonitor.updateData(varDict)
    ...
    def load(self):
        ...
        if not self.strategyLoaded:
            ...
            self.updateMonitor()
            # 加载信号
            self.turtleEngine.initStrategyManager()
    ...
    def init(self):
        """初始化组合"""
        self.ctaEngine.initPortfolio()

    # ----------------------------------------------------------------------
    def start(self):
        """启动组合"""
        self.ctaEngine.startPortfolio()

    # ----------------------------------------------------------------------
    def stop(self):
        """停止组合"""
        self.ctaEngine.stopPortfolio()


########################################################################
class TurtleEngineManager(QtWidgets.QWidget):
    """CTA引擎管理组件"""
    signal = QtCore.Signal(type(Event()))
    ...
    def __init__(self, ctaEngine, eventEngine, parent=None):
        """Constructor"""
        super(TurtleEngineManager, self).__init__(parent)
    ...
    def initUi(self):
        ...
        # 海龟组合
        portfolioManager = TurtlePortfolioManager(self.ctaEngine, self.eventEngine, self)

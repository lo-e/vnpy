...
class BacktestingEngine(object):
    ...
    def initPortfolio(self, filename, portfolioValue=10000000):
        ...
        self.output(u'Portfolio contract code%s' % (self.symbolList))
        self.output(u'Initial value of the portfolio%s' % (portfolioValue))
    ...
    def initListPortfolio(self, l, portfolioValue=10000000):
        ...
        self.output(f"Contract code of the portfolio (total：{len(self.symbolList)}）\n{self.symbolList[:10]}")
        self.output(f"Initial value of the portfolio：{portfolioValue}")
    ...
    def loadData(self):
        ...
        for symbol in self.symbolList:
            ...
            self.output(
                f"{datetime.now()}({index}/{len(self.symbolList)})\t{symbol}Data loading completed, total data volume：{cursor.retrieved}")

        dateList = sorted(dataDict.keys())
        for theDatetime in dateList:
            self.dataDict[theDatetime] = dataDict[theDatetime]

        self.output(u'All data loading completed')

    def runBacktesting(self):
        """运行回测"""
        self.output(f"{datetime.now()}\tStart replaying K-line data")
        ...
        for dt, barDict in self.dataDict.items():
            ...
            if time_cost > log_time:
                ...
                print(f"K-line data playback：{dt}")
            self.currentDt = dt
        ...
        self.output(f"{datetime.now()}\tK-line data playback ends")

    def calculateResult(self, annualDays=240):
        """计算结果"""
        self.output(u'Start counting backtest results')
    ...
    def showResult(self, figSavedPath=''):
        ...
        # Output statistics
        self.output('-' * 30)
        self.output('First trading day:\t%s' % result['startDate'])
        self.output('Last trading day:\t%s' % result['endDate'])

        self.output('Total trading days:\t%s' % result['totalDays'])
        self.output('Profitable trading days:\t%s' % result['profitDays'])
        self.output('Losing trading days:\t%s' % result['lossDays'])

        self.output('Starting capital:\t%s' % self.portfolioValue)
        self.output('Ending capital:\t%s' % formatNumber(result['endBalance']))

        self.output('Total return:\t%s%%' % formatNumber(result['totalReturn']))
        self.output('Annualized return:\t%s%%' % formatNumber(result['annualizedReturn']))
        self.output('Total profit/loss:\t%s' % formatNumber(result['totalNetPnl']))
        self.output('Maximum drawdown: \t%s\t%s' % (formatNumber(result['maxDrawdown']), result['maxDrawdownDate']))
        self.output('Percentage maximum drawdown: %s%%\t%s' % (
        formatNumber(result['maxDdPercent']), result['maxDdPercentDate']))
        # self.output('Maximum drawdown [principal]: \t%s\t%s' % (formatNumber(result['maxDrawdownOrigin']), result['maxDrawdownOriginDate']))
        # self.output('Percentage maximum drawdown [principal]: %s%%\t%s' % (formatNumber(result['maxDdPercentOrigin']), result['maxDdPercentOriginDate']))

        self.output('Total commission:\t%s' % formatNumber(result['totalCommission']))
        self.output('Total slippage:\t%s' % formatNumber(result['totalSlippage']))
        self.output('Total trades:\t%s' % formatNumber(result['totalTradeCount']))

        self.output('Average daily profit/loss:\t%s' % formatNumber(result['dailyNetPnl']))
        self.output('Average daily commission:\t%s' % formatNumber(result['dailyCommission']))
        self.output('Average daily slippage:\t%s' % formatNumber(result['dailySlippage']))
        self.output('Average daily trades:\t%s' % formatNumber(result['dailyTradeCount']))

        self.output('Average daily return:\t%s%%' % formatNumber(result['dailyReturn']))
        self.output('Return standard deviation:\t%s%%' % formatNumber(result['returnStd']))
        ...
        for symbol, l in self.tradeDict.items():
            ...
            for trade in l:
                ...
                if close:
                    ...
                    print('*' * 20)
                    print('%s\t%s volume：%s\tCalculation of daily trading profit and loss data missing' % (self.date, symbol, trade.volume))
                    print('*' * 20 + '\n')
    ...
    def calculateHoldingPnl(self):
        ...
        for symbol, pos in self.posDict.items():
            ...
            if close:
                ...
                print('*' * 20)
                print('%s\t%s pos：%s\tCalculation of profit and loss data for the day is missing' % (self.date, symbol, pos))
                print('*' * 20 + '\n')

...
class PondSignal(object):
    def __init__(self, portfolio, symbol):
        ...
        # 预测信号数据
        self.signal_df = self.portfolio.engine.symbol_signal_dict[self.symbol]
        # 预测信号最后一条数据的datetime
        self.end_prediction_dt = self.signal_df["datetime"].iloc[-1]
    ...
    def generateSignal(self, bar):
        ...
        filtered_data = self.signal_df[self.signal_df["datetime"]
                                       <= target_prediction_dt]
        if not filtered_data.empty:
            if not self.start:
                next_rows = self.signal_df[self.signal_df["datetime"]
                                           > target_prediction_dt]
            ...
            price = target_prediction_data["price"]
            price = float(price)
            volume = target_prediction_data['volume']
            volume = float(volume)
            mean_pnl = target_prediction_data['mean_pnl']
            mean_pnl = float(mean_pnl)
            max_pnl = target_prediction_data['max_pnl']
            max_pnl = float(max_pnl)
            transactions = target_prediction_data['transactions']
            transactions = int(transactions)
            open_volume = self.portfolio.portfolioValue / \
                (len(self.portfolio.engine.symbolList) * bar.close_price)

            if prediction/price > 0.15:
                ...
                if self.pos < 0:
                    ...
                    # Then, buy to open a position self.buy(bar.close_price, abs(open_volume))

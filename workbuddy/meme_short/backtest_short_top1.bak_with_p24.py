...
SIGNAL_FILE = "data/change_20260101_20260812/000000_top1_rise.csv"
...
def ensure_close_map(sym):
    """按币所需 K 区间 [首上榜-30天-5min, 末上榜+24h-5min]。
    完整性按"实际数据起始(上市/拉取覆盖起点)到 end 连续"判定——上市晚的币头部真缺失不算缺失,
    只有区间内出现缺口才重试拉取(最多3次), 仍缺标记 None。"""
    if sym in close_maps:
        return close_maps[sym]
    kc = db[f"{sym.upper()}.{EXCHANGE}"]
    kc.create_index("openTime", unique=True)
    start_ms = first_sig[sym] - LOOKBACK_MS - STEP5   # 创30天新高确认需看 30 天 high
    ...
    for attempt in range(4):  # 0=查库, 1~3=拉取重试
        ...
        ots = sorted(cm.keys())
        if ots:
            eff_start = ots[0]  # 实际数据起始(上市晚的币 = 上市时刻)
            if len(cm) >= (end_ms - eff_start) // STEP5 + 1:
                break
        if attempt >= 3:
            break
        try:
            ...
            print(f"  [fetch] {sym} 第{attempt + 1}次 返回 {len(kl)} 根 "
                  f"[{local_str(start_ms)} ~ {local_str(end_ms)}]", flush=True)
            upsert(kc, kl)
        except Exception as e:
    ...
    if ots:
        eff_start = ots[0]
        expect = (end_ms - eff_start) // STEP5 + 1
    else:
        eff_start, expect = None, 0
    if not ots or len(cm) < expect:
        print(f"  [skip] {sym} 重试3次后仍缺失 {len(cm)}/{expect}, 标记跳过", flush=True)
        close_maps[sym] = None
        return None
...
while tick <= T_END_MS:
        ...
        if tick >= h["open_ms"] + DAY_MS:
            ...
            if tick - h["open_ms"] >= PUMP_DELAY_H * 3600 * 1000 and k["high"] > h1_prev:
                ...
                if TAKE_ENABLE and _peak_p >= TAKE_MIN and (_peak_p - _cur_p) >= TAKE_RETRACE:
                    ...
                    # 未触发 → 开仓时24h前价锚止损(与开仓妖币判定同口径: 价格达 开仓时24h前价 × STOP_FACTOR)
                    stop_px = (h["p24"] * STOP_FACTOR) if h["p24"] else (h["open_px"] * STOP_FACTOR)
                    if k["open"] >= stop_px:
                        reason = "stop_loss"
                        px = k["open"]
                    elif k["high"] >= stop_px:
            ...
            # 开仓时24h前价锚止损: 盘中触及 开仓时24h前close × STOP_FACTOR 即损(与开仓妖币判定同口径)
            stop_px = (h["p24"] * STOP_FACTOR) if h["p24"] else (h["open_px"] * STOP_FACTOR)
            # 止损: 该 K 内价格触及过止损价即触发(不看收盘价)
            if k["open"] >= stop_px:      # 开盘即跳空越过止损 → 按开盘价成交
                reason = "stop_loss"
                px = k["open"]
            elif k["high"] >= stop_px:    # 盘中触及止损 → 按止损价成交
                reason = "stop_loss"
                px = stop_px
        if reason:
            ...
            trades.append([sym, local_str(h["open_ms"]), round(h["open_px"], 6),
                           ...
                           h.get("sim_type", "live"), h.get("weight", 1.0)])
    ...
    while sig_idx < len(signals) and signals[sig_idx][0] <= tick:
        ...
        if TAKER_N:
            ...
            _k24o = close_maps.get(sym, {}).get(t_ms - STEP5 - DAY_MS)
            holdings[sym] = {"open_ms": t_ms, "open_px": px, "sim_type": sim_type,
                             ...
                             "p24": _k24o["close"] if (_k24o and _k24o["close"] > 0) else None,
                             "peak_profit": 0.0}
            n_open += 1
    tick += STEP5
...
with open(OUT_FILE, "w", newline="", encoding="utf-8-sig") as f:
    ...
    for t in sorted(trades, key=lambda r: r[1]):  # 按开仓时间排序
        ...
        w.writerow([t[0], t[1], t[2], t[3], t[4], t[5], t[6], t[7], t[8], round(cum, 4)])

...
def _ensure(sym):
    """预拉补数据专用(2026-08-21 修复): 只触发补拉, 加载后立即释放缓存。
    原实现把 419 币 K 线全驻留 close_maps(15GB+ 卡爆), 主循环内 6h 释放机制来不及救预拉阶段。"""
...
with ThreadPoolExecutor(max_workers=3) as ex:
    ...
    for fut in as_completed(futs):
        ...
        _done_n += 1
        if _done_n % 20 == 0 or _done_n == len(syms_all):
            print(f"  [pre] 预拉进度 {_done_n}/{len(syms_all)} (ok={len(ok_syms)})", flush=True)
...
while tick <= T_END_MS:
    ...
    # 内存释放(2026-08-21 v10): 每 1h 释放闲置币, 窗口 1 天 ——
    #   v9 用 3 天窗口: 2025年信号密 → 3天活跃币163个×45MB=7.3GB → GC 每轮扫 7.3GB → 主循环慢 10 倍。
    #   1 天窗口 → 缓存 ~50-80 币 ≈ 2-3GB → GC 快(定稿窗口 8.5个月仅3.5分钟, 正常速度应 ~15分钟/23个月)
    if tick - _last_cleanup >= 1 * 3600 * 1000:
        _last_cleanup = tick
        _expired = [s for s in list(close_maps.keys())
                    if s not in holdings and s not in permits
                    and tick - last_sig[s] > DAY_MS]
        for _s in _expired:
            close_maps.pop(_s, None)
            series.pop(_s, None)
        if _expired:
            print(f"  [mem] 释放 {len(_expired)} 个闲置币缓存, 当前缓存 {len(close_maps)} 币", flush=True)
    # 平仓检查(先于开仓)
    for sym in list(holdings.keys()):
        ...
        if tick >= (h.get("permit_sig") or h["open_ms"]) + DAY_MS:
            # expire 以 permit 确认时间为基准(2026-08-20 超定): 持仓在 permit+24h 到期, 非开仓+24h; 非permit模式回退open_ms
            reason = "expire"
            px = k["close"]
        elif TAKER_N:
            ...
            if k["high"] >= h1_prev:
                h["broke_flag"] = True   # 持仓期破前高状态锁存(2026-08-20 超定): 突破打开, 加仓/首仓重置关闭
            _sig0 = h.get("permit_sig") or h["open_ms"]
            pump_ok = tick - _sig0 >= PUMP_DELAY_H * 3600 * 1000   # 8h缓冲从permit确认时间起算(2026-08-20 超定)
            ...
            if PUMP_VOL_RATIO and pump_ok and trigger == h1_prev:
                # 量能确认(2026-08-19): 突破当根成交额 >= RATIO × 前N根均额 才算真突破; 无量→假突破, 本tick不止损
                _ots4 = series.get(sym, (None, None))[0]
                _cm4 = close_maps.get(sym, {})
                if _ots4 and _cm4:
                    _i = bisect.bisect_left(_ots4, tick - STEP5)
                    _vs = [_cm4[_ots4[_j]].get("qv", 0.0) or 0.0 for _j in range(max(0, _i - PUMP_VOL_N), _i)]
                    if _vs:
            ...
            if k["open"] >= trigger:                   # 跳空越过 → 按开盘价成交
                ...
                if _vol_ok:
                    reason = "pump_stop" if (pump_ok and trigger == h1_prev) else "stop_loss"
                    px = trigger
            if reason is None:                         # 未触发 或 无量假突破(量能未确认) → 加入当前K + 止盈检查
        ...
        if reason:
            ...
            # 加仓笔: 同一 tick/价格/原因 一次性全平(2026-08-20 超定)
    ...
    while sig_idx < len(signals) and signals[sig_idx][0] <= tick:
        ...
        if PERMIT and sym in permits and permits[sym]["end_ms"] > tick:
            continue              # 活跃 permit 窗口内(2026-08-19 超定): 只有开平仓逻辑(状态机); 妖币禁仓/新permit 一律不评估
        ...
        if RATIO6_MIN or PULSE_W > 1:
            ...
            # 2026-08-21 修复: 必须 ensure 后再取缓存 —— _ensure 预拉后释放缓存(内存修复), 直接 close_maps.get 为空 → px=None → 全挡
        ...
        if not os.environ.get("SKIP_SIM_FILTER", "1"):   # 定稿默认跳过模拟盈亏过滤; 需启用时设 SKIP_SIM_FILTER=0
        ...
        for sym in list(permits.keys()):
            ...
            if tick >= p["end_ms"]:
                if sym not in holdings:
                    ...
                    del permits[sym]
                continue
            if p["n_open"] >= MAX_PERMIT_OPENS or sym in missing_syms:
                continue
            _in_hold = sym in holdings
            if _in_hold:
                # 加仓(2026-08-20 超定): 仅 permit 确认后 8h(PUMP_DELAY_H)内, 且持仓期破前高状态已打开 → 允许继续走开仓条件(加仓)
                if not ADD_ON_BREAK or tick - p["sig_ms"] >= PUMP_DELAY_H * 3600 * 1000:
                    continue
                if not holdings[sym].get("broke_flag"):
                    continue  # 破前高状态未打开 → 本tick不加仓
            if FUNDING_MIN:
                # funding 开仓闸(2026-08-19 超定): tick前最近funding < 阈值 → 本tick不开, 等下一tick
                _fr = funding_at(sym, tick)
                if _fr is None or _fr < FUNDING_MIN:
                    continue
            if CVD_DIVERGE_W:
                # CVD 背离定入场点(2026-08-19 超定): 窗口期内未持仓时逐tick检查, 背离成立(近W根主动买净额<前W根)才开仓; 不成立等下一tick
            ...
            if px is None:
                continue
            if OFF_HIGH_MAX:
                # 开仓价距24h最高价降幅过滤(2026-08-19 超定): 降幅>阈值 = 已从顶部深跌, 追跌不做空
                    ...
                    if _hi9 >= _lo9:
                        ...
                        if _h24_9 > 0:
                            ...
                            if _drop9 > OFF_HIGH_MAX:
                                continue
                            if OFF_HIGH_MIN and _drop9 < OFF_HIGH_MIN:
                                # 最小降幅(2026-08-20 超定): 贴顶猜顶不做空, 等小幅回落确认
                                continue
            if ENTRY_H24:
                ...
                entry_h24 = None
            if _in_hold:
                # 加仓: 合并到现有持仓(同权重), 平仓时一次性全平; 加仓后重置破前高状态
                ...
                holdings[sym] = {"open_ms": tick, "open_px": px, "sim_type": "live",
                                 "max_high": entry_max_high(sym, tick, px),
                                 "weight": p["weight"] / MAX_PERMIT_OPENS,  # 均分权重(2026-08-19 超定): permit确认时weight÷最大开仓数=单次开仓权重, 总敞口不变

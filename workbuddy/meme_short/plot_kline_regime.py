# -*- coding: utf-8 -*-
"""plot_kline_regime.py — 任意 symbol 日线走势图 + regime(BTC代理) 分段着色。

主图: {SYMBOL} 日级 K 线(取自 MongoDB 5m 库) 或回退 close 折线
背景: 按 BTC 代理 regime 连续段着色 (BULL绿/BEAR红/SIDEWAYS灰)
副图: BTC r200 年化斜率 —— 直观展现"市场天气"与价格背景
输出: data/{symbol_lower}_regime_chart.html

用法:
  SYMBOL=SNDKUSDT python plot_kline_regime.py
  (默认 SYMBOL=BTCUSDT, 等价于原 plot_btc_regime.py)
"""
import os
import csv
import json
...
OUT = os.environ.get("OUT", os.path.join(HERE, "data", f"{LOWER}_regime_chart.html"))
URI = os.environ.get("MONGO_URI", "mongodb://127.0.0.1:27017")

# ---------- 1. 读 BTC regime (策略天气代理) ----------
regimes = {}
r200_raw = {}
...
# ---------- 2. 取目标 symbol 日级 OHLC ----------
mongo_map = {}
mode = "line"
try:
    cli = pymongo.MongoClient(URI, serverSelectionTimeoutMS=6000)
    col = cli["Workbuddy_5Min_Db"][f"{SYMBOL}.BINANCE"]
    pipe = [
    ...
    if mongo_map:
        mode = "kline"
    print(f"[ok] MongoDB 取 {SYMBOL} 日级 OHLC: {len(mongo_map)} 天", flush=True)
...
<head>
<meta charset="UTF-8">
<title>__SYM__ 日线 · Regime 分段</title>
...
var option = {
  ...
  legend:{data:['__SYM__','r200年化'], top:4, textStyle:{color:'#c9d1d9'}},
  ...
  series:[
    Object.assign(__MAIN__, {name:'__SYM__'}),
...
with open(OUT, "w", encoding="utf-8") as f:
    f.write(html)
print(f"[done] SYMBOL={SYMBOL} 模式={mode} 天数={len(dates)} regime段={len(segs)} -> {OUT}", flush=True)

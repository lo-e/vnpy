...
# maxDD / ddRatio: 优先取 CSV 末行(逐笔累计的全程值); 旧版13列CSV无ddRatio列时自行重算
if rows and "ddRatio" in rows[0] and "maxDD" in rows[0]:
    max_dd = float(rows[-1].get("maxDD", 0) or 0)
    dd_ratio = float(rows[-1].get("ddRatio", 0) or 0)
else:
    _cum = _peak = _mdd = 0.0
    for p in pts:
        _cum += p["pnl"] * p["w"]
        _peak = max(_peak, _cum)
        _mdd = max(_mdd, _peak - _cum)
    max_dd = _mdd
    dd_ratio = _mdd / total if total > 0 else 0.0
